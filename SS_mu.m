...
%finding steady state values for different values of mu
...
figure(1) %Steady state values for different values of mu
mu_vec = linspace(0.1,1,90);
subplot(221),plot(mu_vec,xroot_vec(:,1))
ylabel('xss')
xlabel('mu')

subplot(222),plot(mu_vec,xroot_vec(:,2))
ylabel('Sss')
xlabel('mu')

subplot(223),plot(mu_vec,xroot_vec(:,3))
ylabel('p1ss')
xlabel('mu')

subplot(224),plot(mu_vec,xroot_vec(:,4))
ylabel('p2ss')
xlabel('mu')
...
for k=1:length(xroot_vec)
    ...
    [Vec,lambda] = eig(Jss);
    %Vec
    lambda_vec(k,1) = lambda(1,1);
    lambda_vec(k,2) = lambda(2,2);
    lambda_vec(k,3) = lambda(3,3);
    lambda_vec(k,4) = lambda(4,4);
    Eigvec_1(:,k) = Vec(:,1);
    Eigvec_2(:,k) = Vec(:,2);
    Eigvec_3(:,k) = Vec(:,3);
    Eigvec_4(:,k) = Vec(:,4);
...
%phase plane plot for stable point
...
for i = 1:20
    %Vmat = [Eigvec_1(:,1) Eigvec_2(:,1) Eigvec_3(:,1) Eigvec_4(:,1)];
    c = inv(Vec)*xin;

    xhat = @(t) c(1)*exp(lambda_vec(1,1)*t)*Vec(1,1) + c(2)*exp(lambda_vec(1,2)*t)*Vec(1,2) + c(3)*exp(lambda_vec(1,3)*t)*Vec(1,3) + c(4)*exp(lambda_vec(1,4)*t)*Vec(1,4) ;
    shat = @(t) c(1)*exp(lambda_vec(1,1)*t)*Vec(2,1) + c(2)*exp(lambda_vec(1,2)*t)*Vec(2,2) + c(3)*exp(lambda_vec(1,3)*t)*Vec(2,3) + c(4)*exp(lambda_vec(1,4)*t)*Vec(2,4) ;
    p1hat = @(t) c(1)*exp(lambda_vec(1,1)*t)*Vec(3,1) + c(2)*exp(lambda_vec(1,2)*t)*Vec(3,2) + c(3)*exp(lambda_vec(1,3)*t)*Vec(3,3) + c(4)*exp(lambda_vec(1,4)*t)*Vec(3,4) ;
    p2hat = @(t) c(1)*exp(lambda_vec(1,1)*t)*Vec(4,1) + c(2)*exp(lambda_vec(1,2)*t)*Vec(4,2) + c(3)*exp(lambda_vec(1,3)*t)*Vec(4,3) + c(4)*exp(lambda_vec(1,4)*t)*Vec(4,4) ;
...
%phase plane plot for saddle point
